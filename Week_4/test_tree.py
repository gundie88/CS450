...
dataset = pd.read_csv(url_data, names=['buying','maint','doors','persons','lug_boot','safety','class'])
...
def train_test_split(dataset):
    training_data = dataset.iloc[:6].reset_index(drop=True)#We drop the index respectively relabel the index
    #starting form 0, because we do not want to run into errors regarding the row labels / indexes
    testing_data = dataset.iloc[6:].reset_index(drop=True)
...
# """
# data_train, data_test, targets_train, targets_test = train_test_split(data, target, 
